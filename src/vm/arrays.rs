...
mod tests {
    ...
    fn test_1d_array_offset_calculation() {
        ...
        let dimensions = vec![Dimension { lower_bound: 1, upper_bound: 5 }];
    ...
    fn test_2d_array_offset_calculation() {
        ...
        let dimensions = vec![
            Dimension { lower_bound: 1, upper_bound: 3 }, // 3 elements
            Dimension { lower_bound: 1, upper_bound: 2 }, // 2 elements
        ];
        let array = Array::new(VariableType::Integer, dimensions);
    ...
    fn test_array_bounds_checking() {
        let dimensions = vec![Dimension { lower_bound: 1, upper_bound: 3 }];
        let array = Array::new(VariableType::Integer, dimensions);
        ...
        // Invalid indices
        assert!(matches!(array.calculate_offset(&[0]), Err(VMError::SubscriptOutOfRange)));
        assert!(matches!(array.calculate_offset(&[4]), Err(VMError::SubscriptOutOfRange)));
    }
    ...
    fn test_array_with_custom_bounds() {
        ...
        let dimensions = vec![
            Dimension { lower_bound: 5, upper_bound: 7 }, // 3 elements
            Dimension { lower_bound: 10, upper_bound: 12 }, // 3 elements
        ];
        let array = Array::new(VariableType::Integer, dimensions);
