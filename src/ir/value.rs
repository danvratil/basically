...
impl Value {
    ...
    pub fn checked_add(self, other: &Value) -> Result<Option<Value>, VMError> {
        match (&self, other) {
            ...
            (Value::Integer(a), Value::SinglePrecision(b)) => Ok(Some(Value::SinglePrecision(*a as f32 + *b))),
            (Value::Integer(a), Value::DoublePrecision(b)) => Ok(Some(Value::DoublePrecision(*a as f64 + *b))),
            ...
            (Value::Long(a), Value::SinglePrecision(b)) => Ok(Some(Value::SinglePrecision(*a as f32 + *b))),
            (Value::Long(a), Value::DoublePrecision(b)) => Ok(Some(Value::DoublePrecision(*a as f64 + *b))),
            (Value::SinglePrecision(a), Value::Integer(b)) => Ok(Some(Value::SinglePrecision(a + *b as f32))),
            (Value::SinglePrecision(a), Value::Long(b)) => Ok(Some(Value::SinglePrecision(a + *b as f32))),
            (Value::SinglePrecision(a), Value::SinglePrecision(b)) => Ok(Some(Value::SinglePrecision(a + b))),
            (Value::SinglePrecision(a), Value::DoublePrecision(b)) => Ok(Some(Value::DoublePrecision(*a as f64 + b))),
            (Value::DoublePrecision(a), Value::Integer(b)) => Ok(Some(Value::DoublePrecision(a + *b as f64))),
            (Value::DoublePrecision(a), Value::Long(b)) => Ok(Some(Value::DoublePrecision(a + *b as f64))),
            (Value::DoublePrecision(a), Value::SinglePrecision(b)) => Ok(Some(Value::DoublePrecision(a + *b as f64))),
            (Value::DoublePrecision(a), Value::DoublePrecision(b)) => Ok(Some(Value::DoublePrecision(a + b))),
            (Value::String(a), Value::String(b)) => Ok(Some(Value::String(format!("{}{}", a, b)))),
            _ => Err(VMError::TypeMismatch(self.type_name().to_string(), other.type_name().to_string())),
        }
    }
